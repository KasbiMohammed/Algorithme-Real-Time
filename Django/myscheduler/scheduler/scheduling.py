...
def update_task_metrics(tasks, current_time):
    for task in tasks:
...
def get_active_tasks(tasks, current_time):
    active_tasks = []
    for task in tasks:
...
def dm_schedule(tasks, simulation_time):
    ...
    while current_time < simulation_time:
        update_task_metrics(tasks, current_time)
        active_tasks = get_active_tasks(tasks, current_time)
        ...
        if active_tasks:
            ...
            if current_task.remaining_time <= 0:
                current_task.next_release += current_task.period
        else:
            timeline.append(("Idle", current_time, current_time + 1))
...
def edf_schedule(tasks, simulation_time):
    ...
    while current_time < simulation_time:
        ...
        if active_tasks:
            # Sort by absolute deadline (EDF priority)
...
def llf_schedule(tasks, simulation_time):
    ...
    while current_time < simulation_time:
        ...
        if active_tasks:
            # Update and sort by laxity (LLF priority)
